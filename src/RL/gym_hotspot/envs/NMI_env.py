...
class NMI_v0(gym.Env):
    ...
    def step(self, action):
        ...
        CRPS = get_CRPS(
            probability_distribution=flux_after_DA, true_value=self.flux_true
        )
        reward = -CRPS
    ...
    def reset(self, seed=None, options=None):
        ...
        selected_flux = rng.uniform(low=0.2, high=0.3, size=1)
        self.flux_true = (selected_flux * self.dx * self.dy)[
            0
        ]  # flux [g/(s*m2)] * area of grid cell [m2] = flux [g/s]
...
class NMI_v1(gym.Env):
    """
    reward = KL divergence with prior before observation
    """
    ...
    def step(self, action):
        ...
        flux_before_DA = deepcopy(self.flux)
        flux_after_DA = perform_EnKF_with_gaussian_plume_model(
            flux=self.flux,
            observation=next_concentration,
            ensemble_size=self.ensemble_size,
        ...
        KLdiv = get_kldiv(posterior=flux_after_DA, prior=flux_before_DA)
        reward = KLdiv
...
class NMI_v2(gym.Env):
    """
    reward = KLdiv with initial prior
    """
    ...
    def step(self, action):
        ...
        KLdiv = get_kldiv(posterior=flux_after_DA, prior=self.flux_prior)
        reward = KLdiv
...
class NMI_v3(gym.Env):
    ...
    def step(self, action):
        ...
        entropy = get_entropy(flux_after_DA)
        reward = -entropy
